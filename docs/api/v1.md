...
### `POST /debug/:debugID/spiceTypes/:function`

Identifies a local pointer on the [`Function`](#function) as a null terminated array (string), or an array of a specific size.

Request:
```
{
    variableAddress: integer; // unique address of a Variable within the Function
    spiceType: "none" | "string";
}
{
    variableAddress: integer; // address of a pointer Variable within the Function
    spiceType: "array";

    /* If count is a static user-defined integer.
     * If count is another Variable defined locally. */
    countType: "static" | "variable";

    /* If countType="static" then this is the count of the array.
     * If countType="variable" this is a variableAddress of another variable Variable defined locally. */
    count: integer;
...
Returns:
- `200`
    - The spice type has been set.
- `400`: [`Error`](#error)
    - One of the **variableAddress** parameters did not point to anything.
    - The [`Variables`](#variable) pointed to were not of the correct type.
