...
### `POST /debug/:debugID/functions/:function/locals/:address`
...
{   spiceType: "none";  }
{   spiceType: "string";}
{   spiceType: "array"; count: integer;  }
{   spiceType: "vector"; count: integer; }
```
...
- `400`: [`Error`](#error)
    - The **address** or **count** parameters did not point to anything.
    - The [`Variables`](#variable) pointed to were not of the correct type (pointer and integer).

### `POST /debug/:debugID/types/:type/:offset`

Identifies a field at an offet on the [`Struct`](#types) as a null terminated array (*string*), or an array of a specific size.
...
- `200`
    - The spice type has been set.
- `400`: [`Error`](#error)
    - The **offset** or **count** parameters did not point to anything.
    - The [`Variables or Fields`](#variable) pointed to were not of the correct type.
