...
Returns execution time, program output, and an [execution id](#execution) when the program encounters a breakpoint,
or with no execution id if the program terminates.
...
Responses:
 - `200`
    - Returns a stream of {[Trace](#trace)[]}. The body begins with an open bracket `[` on its own line. Each Trace is on its own line. If the execution is still `executing`, new lines will be streamed as they are produced. When the execution is finished the last Trace object has `type=end` and describes how the execution terminated. The final line of the body contains `]`, then the response is closed.
 - `400`: Invalid execution id
    - Returns {[Error](#error)]}
 - `404`: Execution id not found
    - Returns {[Error](#error)]}
...
```
{
    ...
    contents: {string|string[]}:
        If type="file", a string of the base64 encoded file contents
        If type="directory", an array of File names inside the directory
}
```
...
```
{
   ...
   type {string}: Type of execution. Either `function` or `process`
   status {string}: Either `pending`, `executing`, `stopped`, or `done`
   executionTime {integer}: Nanoseconds
   data {object}: Data specific to this type
      if `type=function`: {
...
```
{
   ...
   type {integer}: Type of trace: `0`=instruction (state changes), `1`=output (to stdout), `2`=termination (data about how the execution ended)
   line {integer}: Line number that produced this trace
   data {object}: Data specific to the trace type
      if `type=0`: {
...
```
{
   ...
   parameters {object[]}: Array of function parameter objects--{name {string}, type {SourceType}}
}
```
### AttachInfo
```
...
```
{
  ...
  type {string}: Type of the variable as it is defined in the source code.
  data {any}: The data within the variable.
}
```
