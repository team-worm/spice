...
may have some fields omitted for brevity.
 ...
 1. Attach to binary at path `/home/hello`:
    a. Request `POST /api/v1/debug/attach/bin/home/hello`
    b. Response: AttachInfo `{id: 1, sourcePath"/home/src"}`
 2. Get directory info and source code
    a. Request `GET /api/v1/filesystem/home/src`
    b. Response: File `{fType: "directory", contents:[{fType: "file", name: "hello.cpp"}]}`
    c. Request `GET /files/home/src/hello.cpp`
    d. Response: Binary contents of `hello.cpp`
 3. Get list of all functions
    a. Request `GET /api/v1/debug/1/functions`
    b. Response: List of Functions `[{address: 54, name:"add" sourcePath: "/home/src/hello.cpp", parameters: [{id: 0, name: "a"}, {id: 1, name: "b"}], localVariables=[{id: 2, name: "sum"}]}]`
 4. Set breakpoint on function
    a. Request `PUT /api/v1/debug/1/breakpoints/54`
    b. Response: Breakpoint `{function: 54}`
 5. Execute program
    a. Request: `POST /api/v1/debug/1/execute`, body: `{args: "2 3"}`
    b. Response: Execution `{id: 16, eType: "process", status: "executing"}`
    c. Debugger begins executing the program, tracking and storing only stdout because this is a `process` execution.
 6. Get process output
    a. Request `GET /api/v1/debug/1/executions/16/trace`
    b. Response: List of traces:
       ```
       [
 ...
 7. Get execution information and function information and source code (so we can switch to "debugger view" on the client)
    a. Request `GET /api/v1/debug/1/executions/17
    b. Response: Execution `{id: 17, eType: "function", status: "executing", data: {function: 54}}`
    c. Request `GET /api/v1/debug/1/functions/54`
    d. Response: Function `{address: 54, name:"add" sourcePath: "/home/src/hello.cpp", parameters: [{id: 0, name: "a"}, {id: 1, name: "b"}], localVariables=[{id: 2, name: "sum"}]}`
    e. Request `GET /files/home/src/hello.cpp`
    f. Response: Binary contents of `hello.cpp`
 8. Get function trace
    a. Request `GET /api/v1/debug/1/executions/17/trace`
     b. Response: List of traces:
       ```
       [
